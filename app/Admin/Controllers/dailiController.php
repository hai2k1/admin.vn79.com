 ...
 * @property int ad_tag
 */
class dailiController extends BaseUserController
{
    ...
    public function form(): Form
    {
        return Form::make(User::with(['roles']), function (Form $form) {
            ...
            $form->radio('status')->options(['Tài khoản bị đóng băng', 'Tài khoản bình thường'])->default(1);
            $form->currency('money','tiền')->symbol('VNĐ');
            $form->select('gender')
    ...
    {
        return Grid::make(User::with(['roles', 'department'])->whereRelation('roles','name','Đại lí'), function (Grid $grid) {
            $grid->column('id');
            $grid->column('key');
